...
exports.findAll = async (req, res) => {
  try {
    const currency = await Currency.findAll(); // Consulta todos
    res.status(200).json(currency); // Devuelve el listado como respuesta
  } catch (error) {
...
exports.update = async (req, res) => {
  try {
    ...
    const [updatedRowsCount, updatedData] = await Currency.update(data, {
      where: { id: id },
...
exports.delete = async (req, res) => {
  try {
    ...
    const deletedRowCount = await Currency.destroy({
      where: { id: id },
    });
