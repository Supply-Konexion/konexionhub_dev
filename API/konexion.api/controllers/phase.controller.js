...
const Phase = require("../db/models/phase.model");

exports.findAll = async (req, res) => {
  try {
    const phase = await Phase.findAll(); // Consulta todos
    res.status(200).json(phase); // Devuelve el listado como respuesta
  } catch (error) {
...
exports.update = async (req, res) => {
  try {
    ...
    const [updatedRowsCount, updatedData] = await Phase.update(data, {
      where: { id: id },
...
exports.delete = async (req, res) => {
  try {
    ...
    const deletedRowCount = await Phase.destroy({
      where: { id: id },
    });
