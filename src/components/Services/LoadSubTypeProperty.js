...
export default function LoadSubTypeProperty(props) {
  ...
  useEffect(() => {
    const consultar = async () => {
      await axios
        .get(`${urlServices}sub-type-property/list`, {
          headers: {
        ...
        .then((response) => {
          ...
          if (!props.disabled) {
            let result = response.data.filter(
              (filtered) =>
                Number(filtered.typePropertyId) === Number(props.idTypeProperty)
            );
    ...
    consultar();
  }, [urlServices, props.idTypeProperty]);
