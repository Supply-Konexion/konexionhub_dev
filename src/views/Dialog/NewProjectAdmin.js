...
export default function NewProjectAdmin(props) {
  ...
  const [informationPlans, setInformationPlans] = useState([]);
  ...
  const submitForm = async (e) => {
    ...
    const imageHeader = [values.headerImage];
    ...
    try {
      ...
      if (imageHeader.length > 0) {
        imagesPromises.push(
          uploadImage("headerImage", imageHeader, response.data.projectPost.id)
        );
      }

      if (dataImageSlider.length > 0) {
        imagesPromises.push(
          uploadImage("images", dataImageSlider, response.data.projectPost.id)
        );
      }

      if (informationPlans.length > 0) {
        imagesPromises.push(
          uploadImage("planos", informationPlans, response.data.projectPost.id)
        );
      }
  ...
  const uploadImage = async (value, images, id) => {
    ...
    const addFilesToFormData = (images) => {
      images.forEach((image) => {
        if (value === "planos") {
          if (image.image.imageUpload && image.image.imageUpload !== "delete") {
    ...
    try {
      await addFilesToFormData(images);
  ...
  const handleRemoveClick = (i) => {
    const list = [...informationPlans];
    list.splice(i, 1);
    setInformationPlans(list);
  };
  ...
  const handleDeleteImageSlider = (index) => {
    setDataImageSlider((prevImages) =>
      ...
      <Grid item xs={12} md={6}>
        {successInsert ? (
          <Alert
            ...
            sx={{ fontSize: 16 }}
          >
            <b>Proyecto registrado con éxito.</b> Salir del registro.
          </Alert>
        ) : loaders ? (
          <center>
            <img
          ...
          <Grid item xs={12} sm={12} md={12} sx={{ padding: 2 }}>
            ...
            >
              <img
                ...
                style={{
                  width: "100%",
                  height: 230,
                  objectFit: values.headerImage?.newImage ? "cover" : "contain",
                }}
              />
              <b
                style={{
                  ...
                  color: "red",
                  cursor: "pointer",
                  display: values.headerImage?.newImage ? "block" : "none",
                }}
            ...
            <center>
              <Button
                ...
                sx={{ marginTop: 1 }}
                disabled={values.headerImage?.newImage ? true : false}
                size="small"
              >
                Subir imágen
                <VisuallyHiddenInput
          ...
          <Grid item xs={12} sm={12} md={12} sx={{ marginBottom: 2 }}>
            <Grid container>
              <Grid item xs={12} sm={12} md={12} sx={{ padding: "0 10px" }}>
                <Grid container>
                  {dataImageSlider.length > 0 ? (
                    dataImageSlider.map((imageUrl, index) => (
                      <Grid item xs={12} md={3} key={index}>
                        ...
                        >
                          ...
                          <b
                            ...
                            onClick={() => handleDeleteImageSlider(index)}
                          >
                            Eliminar&nbsp;
                            <Close sx={{ fontSize: 14 }} />
                          </b>
