...
import { cardBodyStyle, ButtonStyle0, ButtonExit } from "components/cardBodyStyle";

const useStyles = makeStyles(cardBodyStyle);

export default function NewCountriesAdmin(props) {
  ...
  const submitForm = (e) => {
    ...
    if (values.name.length === 0) {
      ...
      setMensaje('El campo "Code" es obligatorio.');
      setOpenAlert(true);
      setErrorAlert("error");
    } else if (values.prefix.length === 0) {
      setValues({ ...values, errorPrefix: true });
      setMensaje('El campo "Prefijo telefónico" es obligatorio.');
      setOpenAlert(true);
      setErrorAlert("error");
    } else {
      const dataValue = {
      ...
      axios
        .post(`${urlServices}location/countries`, dataValue, {
          headers: {
          ...
          <Fragment>
            <Grid container spacing={1} className={classes.containerProfile}>
              <Grid item xs={12} sm={12}>
                ...
                <div className={classes.titleCardFilter}>Nuevo país</div>
